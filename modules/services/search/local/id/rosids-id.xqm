...
declare %private function rosids-id:filter-getty($terms, $type) {
    switch ($type)
            case "worktypes"
            case "styleperiods"
            case "techniques"
...
declare %private function rosids-id:id-getty($query as xs:string, $type as xs:string) {
    let $terms := rosids-id:filter-getty(( collection($app:global-getty-aat-repositories)//vp:Subject[@Subject_ID = $query], collection($app:global-getty-tgn-repositories)//vp:Subject[@Subject_ID = $query] ), $type)
    return 
        if($terms)
        then (
            for $term in $terms
                let $authority := substring-after(util:collection-name($term), '/db/resources/services/repositories/global/externalmirrors/getty/')
